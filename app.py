...
with tab1:
    ...
    with col1:
        ...
        if source_input: st.session_state["source_raw_input"] = source_input
    ...
    with col3:
        ...
        if destination_input: st.session_state["destination_raw_input"] = destination_input
    ...
    skip_src_check = str(source_input).strip().lower() == "any"
    skip_dst_check = str(destination_input).strip().lower() == "any"
...
with tab2:
    ...
    for i, rule in enumerate(rules_data):
        ...
        if sig in seen_rules:
            insight_rows.append((f"🔁 **Duplicate Rule** at index {i + 1}: same action, protocol, source, destination, and port.", [i + 1]))
        else:
            seen_rules.add(sig)
        ...
        if rule["srcCidr"] == "Any" and rule["destCidr"] == "Any" and rule["destPort"].lower() == "any" and rule["protocol"].lower() == "any":
            if (is_allow_any and is_last) or (is_deny_any and is_penultimate):
                continue
            else:
                insight_rows.append((f"⚠️ **Broad Rule Risk** at index {i + 1}: `{rule['policy'].upper()} ANY to ANY on ANY`", [i + 1]))

        for j in range(i):
            if rule_covers(rules_data[j], rule):
                insight_rows.append((f"🚫 **Shadowed Rule** at index {i + 1}: shadowed by rule at index {j + 1}.", [j + 1, i + 1]))
                break

        if i < len(rules_data) - 1:
            next_rule = rules_data[i + 1]
            same_base = all(rule[f] == next_rule[f] for f in ["policy", "srcCidr", "destCidr"])
            if same_base:
                if rule["destPort"] != next_rule["destPort"] and rule["protocol"] == next_rule["protocol"]:
                    insight_rows.append((f"🔄 **Merge Candidate** at index {i + 1} & {i + 2}: same src/dst/action, different ports.", [i + 1, i + 2]))
                    ...
                    if rule["destPort"].lower() != "any" and next_rule["destPort"].lower() != "any":
                        continue
                    insight_rows.append((f"🔄 **Merge Candidate** at index {i + 1} & {i + 2}: same src/dst/ports/action, different protocol.", [i + 1, i + 2]))
...
| Term                  | Description                                                                  |
|-----------------------|------------------------------------------------------------------------------|
| 🔁 **Duplicate Rule** | Rule is identical to a previous one                                          |
| 🔄 **Merge Candidate**| Rules could be combined (one field differs like ports or protocol)           |
| ⚠️ **Broad Rule Risk**| `ANY` rule early in list that may block others                               |
| 🚫 **Shadowed Rule**  | Rule is unreachable due to a broader match above                             |
""")
...
with tab4:
    ...
    if filtered_grps:
        ...
        if selected_group in group_map:
            member_objs = [object_map[oid] for oid in group_map[selected_group].get("objectIds", []) if oid in object_map]
