...
# with st.sidebar.expander("🔽 Fetch Data", expanded=False):
...
#from utils.helpers import safe_dataframe, get_object_map, get_group_map, id_to_name
#from utils.match_logic import  is_exact_subnet_match, resolve_to_cidrs_supernet_aware,  build_object_location_map
from streamlit_searchbox import st_searchbox
from streamlit_extras.customize_running import center_running
...
def evaluate_rule_scope_from_inputs(source_cidrs, dest_cidrs, obj_location_map):
    ...
    # ðŸ”§ VPN rules: required if at least one pair from different VPN-enabled locations
    vpn_needed = any(
        dst != src and dst in dst_vpn_locs and src in src_vpn_locs
        for src in src_vpn_locs for dst in dst_vpn_locs
    )

    # ðŸ”§ Local rules: show if common location or one side is unmapped
    local_needed = (
        bool(shared_locs)
    ...
    # âœ… Flatten local rule locations to just network names
...
def fetch_updated_vpn_rules(base_url, headers, org_id):
    url = f"{base_url}/organizations/{org_id}/appliance/vpn/firewallRules"
    resp = requests.get(url, headers=headers)
...
if st.session_state["Fetch_DATA_Expand_COLLAPSED"] == False:
    ...
    with st.sidebar.expander("🔽 Load Data", expanded=st.session_state["Fetch_DATA_Expand"]):
                ...
                with st.spinner("🔄 Fetching all API data..."):
                    try:
                        ...
                        if not fetched:
                            ...
                            try:
                                extended_result = fetch_meraki_data_extended(update_progress=update_progress)
                                if st.session_state.get("cancel_extended_fetch"):
                                    st.info("⛔ Fetch cancelled before completion.")
                                    st.session_state["extended_data"] = None
                                    st.session_state["object_location_map"] = {}
        ...
        if st.session_state.get("snapshot_ready"):
            b64 = base64.b64encode(
                st.session_state["snapshot_str"].encode()
            ).decode()

            # HTML runs inside an iframe where <script> is allowed
            components.html(
                f"""
                <html>
                <body>
                    ...
                    download='{st.session_state["snapshot_filename"]}'></a>
...
with st.container():
    ...
    with col_left:
        st.markdown(" 📘-🔎-🛡️-🧠 - ➕ Choose the module:")
        tab_names = ["📘 Overview", "🔎 Search Object or Group", "🛡️ Search in Firewall and VPN Rules", "🧠 Optimization Insights", "➕ Add VPN and Firewall Rules"]
...
if selected_tab == "📘 Overview":
    ...
    if not data_loaded:
        with st.expander("📘 Introduction", expanded=True):
            ...
            - ➕ **Add VPN and Firewall Rules**: Create new rules for VPN and local Firewalls.
...
""")
...
elif selected_tab == "➕ Add VPN and Firewall Rules":
    ...
    if data_loaded:
        ...
        with st.sidebar.expander("🔑 Log-in", expanded=st.session_state.get("expand_login_section", True)):
        ...
        with st.sidebar:
            if st.button("🚀 Deploy Changes", key="deploy_button") and preview_tables and st.session_state["expand_location"] == True:
                ...
                for loc, rules in preview_tables.items():
                    if st.session_state["rule_type"] == "VPN":
                        url = f"https://api.meraki.com/api/v1/organizations/{org_id}/appliance/vpn/firewallRules"
                    else:
                        net_id = network_map.get(loc)
                    ...
                    try:
                        ...
                        cleaned_rules = [r for r in rules if not r.get("marked_for_deletion", False)]

                        if cleaned_rules and is_default_rule(cleaned_rules[-1]):
                            filtered_rules = cleaned_rules[:-1]
                        else:
                            filtered_rules = cleaned_rules
                        resp = requests.put(
                            url,
                            headers=headers,
                            json={"rules": filtered_rules},
                        )
                        if resp.ok:
                            st.session_state.setdefault("rule_log", []).append(f"✅ {loc}: Deployed (Status {resp.status_code})")
                            successful_deployments.append(loc)
                        else:
                            st.code(json.dumps({"rules": rules}, indent=2), language="json")
        ...
        with st.sidebar.expander("🎯 Target Locations", expanded=st.session_state.get("expand_location", False)):
            selected_rule_filter = st.selectbox("Search Rule by Comment", ["<Show All>"] + rule_keys_sorted)
            st.session_state["selected_rule_key"] = selected_rule_filter if selected_rule_filter != "<Show All>" else None
            ...
            if selected_rule_filter != "<Show All>":
                ...
                # Default: include only those with any rules
        ...
        with st.sidebar:
            ...
            if st.button("🔄 Reset Rules", key="reset_button"):
                st.session_state["preview_tables"] = {}
                st.session_state["show_table"] = False
                st.session_state["rule_type"] = ""
                st.success("All preview rule changes have been reset.")

        # --- MAIN LAYOUT ---
        #st.title("➕ Add Firewall or VPN Rule")
            ...
            for loc in selected_locations:
                ...
                if new_rule:
                    ...
                    if insert_position == "Bottom":
                        rule_list.insert(default_index, new_rule)
                ...
                #if st.session_state.get("persist_rule"):
                if is_vpn:
                ...
                df = pd.DataFrame([
                    {
                        ...
                        "is_deleted": r.get("marked_for_deletion", False)
        ...
        with st.expander("➕ Parameters", expanded=True):
            ...
            with col0:
                operation_mode = st.radio("Operation", ["Add", "Delete"], horizontal=True, key="operation_mode")
            with col1:
            ...
            with col2:
                rule_options = []
                match_policy, match_comment = None, None

                if operation_mode == "Add" and insert_position in ["Above", "Below"]:
                    rule_sets = []
                    first_loc_rules = []
                    ...
                    for idx, loc in enumerate(selected_locations):
                        ...
                        rule_set = set((r.get("policy", ""), r.get("comment", "")) for r in rules)
                        rule_sets.append(rule_set)

                        if idx == 0:
                            first_loc_rules = [(r.get("policy", ""), r.get("comment", "")) for r in rules]
                    ...
                    selected_rule = st.selectbox("Match Existing Rule", rule_options, index=0 if rule_options else None)
                    if selected_rule:
                    ...
                    selected_rule = st.selectbox("Select Rule to Delete", rule_options, index=0 if rule_options else None)
                    if selected_rule:
                        match_policy, match_comment = selected_rule.split(" - ", 1)
            ...
            if operation_mode == "Add":
                ...
                rule_prepopulate = {}
                selected_filter_rule = st.session_state.get("selected_rule_key")  # or pass this as a variable
                if selected_filter_rule and " - " in selected_filter_rule:
                    policy_part, comment_part = selected_filter_rule.split(" - ", 1)
                    rule_src = None
                    for loc in all_locations:  # include all locations, not just selected
                ...
                for r in rules:
                    if r.get("comment") == match_comment and r.get("policy", "").lower() == match_policy.lower():
                        preview_row = {
                            "Policy": r.get("policy", "").upper(),
                            "Comment": r.get("comment", ""),
                            "Source": r.get("srcCidr", ""),
                            "Source Port": r.get("srcPort", ""),
                            "Destination": r.get("destCidr", ""),
                            "Destination Port": r.get("destPort", ""),
                            "Protocol": r.get("protocol", ""),
                            "is_new": False,
                            "is_deleted": True,
